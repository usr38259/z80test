...
#define IN(Z,port)          io_read_byte(port)

/***************************************************************
 * Output a byte to given I/O port
 ***************************************************************/
//#define OUT(Z,port,value)   (Z)->io->write_byte(port, value)
#define OUT(Z,port,value)   io_write_byte(port, value)

/***************************************************************
 * Read a byte from given memory location
 ***************************************************************/
//#define RM(Z,addr)          (Z)->program->read_byte(addr)
#define RM(Z,addr)          read_byte(addr)
...
#define WM(Z,addr,value)    program_write_byte((UINT32)(addr), (UINT8)(value))
...
//	return z80->direct->read_decrypted_byte(pc);
	return direct_read_decrypted_byte(pc);
}
...
//	return z80->direct->read_raw_byte(pc);
	return direct_read_raw_byte(pc);
}
...
//	return z80->direct->read_raw_byte(pc) | (z80->direct->read_raw_byte((pc+1)&0xffff) << 8);
	return direct_read_raw_byte(pc) | (direct_read_raw_byte((pc+1)&0xffff) << 8);
}
...
//		SZHVC_sub = global_alloc_array(UINT8, 2*256*256);
		SZHVC_add = (UINT8 *) SZHVC_add_array;
		SZHVC_sub = (UINT8 *) SZHVC_sub_array;
